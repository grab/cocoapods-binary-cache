    ...
    class Podfile
        module DSL
            ...
            def enable_devpod_prebuild
                DSL.enable_prebuild_localpod = true
            end

            def set_devpod_prebuild_unbuilt(list)
                DSL.unbuilt_dev_pods = Set.new(list)
                DSL.unbuilt_pods = DSL.unbuilt_vendor_pods.merge(DSL.unbuilt_dev_pods)
            end

            def set_devpod_is_prebuild_job(flag)
                DSL.devpod_is_prebuild_job = flag
            end

            def set_vendorpod_prebuild_unbuilt(list)
                DSL.unbuilt_vendor_pods = Set.new(list)
                DSL.unbuilt_pods = DSL.unbuilt_vendor_pods.merge(DSL.unbuilt_dev_pods)
            end

            private
            def self.add_prebuild_unbuilt(list)
            ...
            private
            class_attr_accessor :enable_prebuild_localpod
            self.enable_prebuild_localpod = false
            ...
            private
            class_attr_accessor :devpod_is_prebuild_job
            self.devpod_is_prebuild_job = false
        end
    end
end
...
Pod::HooksManager.register('cocoapods-binary-cache', :pre_install) do |installer_context|
    ...
    def verify_devpod_checksum(sandbox, lock_file)
    ...
    if Pod::Podfile::DSL.enable_prebuild_localpod
        BenchmarkShow.benchmark { verify_devpod_checksum(prebuild_sandbox, lockfile) }
        if !Pod::Podfile::DSL.devpod_is_prebuild_job
            Pod::Podfile::DSL.add_prebuild_unbuilt(Pod::Prebuild::CacheInfo.cache_miss_local_pods)
        end
    end
...
Pod::HooksManager.register('cocoapods-binary-cache', :post_install) do |installer_context|
    next unless Pod::Podfile::DSL.enable_prebuild_localpod and installer_context.sandbox.instance_of?(Pod::PrebuildSandbox)
